...
// Pattern: match known error patterns â†’ return short human-readable string.
...
export function parseChainError(raw) {
  ...
  // â”€â”€â”€ Known chain errors â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  if (s.includes('duplicate node for plan')) return 'Node is already in this plan';
  if (s.includes('duplicate provider')) return 'Provider already registered â€” use Update';
  ...
  if (s.includes('invalid price')) return 'Price mismatch â€” node may have changed rates';
  if (s.includes('invalid status inactive')) return 'Plan is inactive â€” activate first';
  ...
  if (s.includes('account sequence mismatch') || s.includes('incorrect account sequence')) {
    return 'Chain busy â€” sequence mismatch after 5 retries. Wait a moment and try again.';
  }
  if (s.includes('out of gas')) return 'Transaction out of gas';
  if (s.includes('timed out')) return 'Transaction timed out';

  // â”€â”€â”€ Extract desc from RPC error â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/**
 * Safely serialize a TX result (handles BigInt â†’ Number conversion).
...
export function txResponse(result) {
  return {
    ...
    // events can contain BigInt block heights â€” strip them by JSON-roundtrip via toJSON
...
//   typeRe   â€” RegExp the event.type must match (e.g. /plan/i)
//   keys     â€” attribute key names to accept (e.g. ['plan_id','id'])
