...
import { currentSession, isMultiUser } from './session.js';
...
// Multi-user: prefer the per-request session (from AsyncLocalStorage). Falls
// back to the module-level wallet for single-user / bootstrap mode.
export function getAddr() {
  const s = currentSession();
  return s ? s.addr : _addr;
}
export function getProvAddr() {
  const s = currentSession();
  return s ? s.provAddr : _provAddr;
}
export function isWalletLoaded() {
  return !!(currentSession() || _addr);
}
...
export async function initWallet(mnemonic) {
  ...
  // Persist address only (not mnemonic) for UI display. In multi-user mode
  // we skip this so one user's login doesn't pin the server's default wallet.
  if (!isMultiUser()) {
