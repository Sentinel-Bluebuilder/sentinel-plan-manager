...
export function initSession(dataDir) {
  ...
  try {
    ...
    // enforce 0600 on Linux/macOS; chmod is a no-op/ENOSYS on Windows, which is fine
    try {
      chmodSync(keyFile, 0o600);
    } catch (e) {
      console.warn(`[session] chmod on session key file failed (ok on Windows): ${e.message}`);
    }
...
export async function sessionFromMnemonic(mnemonic) {
  ...
  session = {
    ...
    async resetClient() {
      ...
      try {
        ...
        console.warn(`[session] disconnect on reset failed: ${e.message}`);
      }
  ...
  while (cache.size >= CACHE_MAX) {
    ...
    cache.delete(oldest);
    // The evicted session owns a live SigningStargateClient (WebSocket). Dropping
    // it from the map alone leaks the socket — disconnect it explicitly.
    try {
      evicted?._client?.disconnect?.();
    } catch (e) {
      console.warn(`[session] disconnect on LRU eviction failed: ${e.message}`);
    }
  }
 ...
 */
export function keplrSessionFromAddress(addr, pubkeyB64, kind = 'keplr') {
  if (!addr || !addr.startsWith('sent1')) throw new Error('Invalid Keplr address');
  // Cache key includes kind so a privy session and keplr session for the same
  // bech32 address (rare but possible) don't collide.
  const cacheKey = `${kind}:${addr}`;
  let session = keplrCache.get(cacheKey);
  if (session) {
    keplrCache.delete(cacheKey); keplrCache.set(cacheKey, session);
  ...
  keplrCache.set(cacheKey, session);
  return session;
}

export function dropKeplrSessionFromCache(addr) {
  // Drop both kinds — caller may not know which bucket the session lives in.
  // Map.delete never throws, so no guard is needed.
  keplrCache.delete(`keplr:${addr}`);
  keplrCache.delete(`privy:${addr}`);
}
...
export function parseCookies(header) {
  ...
  for (const part of header.split(';')) {
    ...
    // A malformed percent-escape (e.g. a stray "%" from a truncated cookie)
    // makes decodeURIComponent throw a URIError, which would crash the async
    // session middleware. Fall back to the raw value instead.
    if (name) {
      try {
        out[name] = decodeURIComponent(val);
      } catch (e) {
        console.warn(`[session] cookie "${name}" decode failed, using raw value: ${e.message}`);
        out[name] = val;
      }
    }
  }
...
export function parseKeplrToken(token) {
  ...
  try {
    ...
    console.warn(`[session] parseKeplrToken base64url decode failed: ${e.message}`);
    return null;
  }
  const expect = createHmac('sha256', _key).update(payload).digest('base64url');
  // Constant-time compare so an attacker can't byte-by-byte forge the MAC via
  // response-timing. timingSafeEqual requires equal-length buffers, so reject a
  // wrong-length mac up front (a length mismatch already means tamper).
  const expectBuf = Buffer.from(expect, 'utf8');
  const macBuf = Buffer.from(mac, 'utf8');
  if (expectBuf.length !== macBuf.length || !timingSafeEqual(expectBuf, macBuf)) return null;
...
export function dropSessionFromCache(mnemonic) {
  // Only hashMnemonic can throw here (e.g. a non-string mnemonic). If it does,
  // the wallet stays in the LRU cache — log it so the lingering session is
  // traceable rather than silently leaked.
  try {
    const key = hashMnemonic(mnemonic);
    cache.delete(key);
  } catch (e) {
    console.warn(`[session] dropSessionFromCache failed — wallet may remain cached: ${e.message}`);
  }
}
