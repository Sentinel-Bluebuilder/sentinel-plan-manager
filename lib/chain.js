 ...
 * Multi-user: if a per-request session is active (AsyncLocalStorage), use
 * that session's own client + registry. Falls back to the module-level
 * wallet for single-user / bootstrap mode.
 *
 ...
 *
 * Multi-user: each session has its own broadcastQueue so different users
 * don't block each other. Same user's concurrent broadcasts still serialize
 * (required for correct sequence numbers).
 *
...
export function safeBroadcast(msgs, memo) {
  const s = currentSession();
  const queueOwner = s || { broadcastQueue: _broadcastQueue };
  const p = queueOwner.broadcastQueue.then(() => _safeBroadcastInner(msgs, memo));
  queueOwner.broadcastQueue = p.catch(() => {});
  if (!s) _broadcastQueue = queueOwner.broadcastQueue;
  return p;
}
