...
export function safeBroadcast(msgs, memo, opts) {
  ...
  // The queue sentinel must always resolve so a failed broadcast doesn't
  // permanently block the next one. The rejection is preserved on `p` (returned
  // to the caller); here we only reset the queue, logging so an unexpected
  // failure leaves a trace rather than vanishing.
  s.broadcastQueue = p.catch((err) => {
    console.warn('[TX] broadcast queue reset after error:', err.message);
  });
  return p;
}
...
async function buildKeplrSignDoc(session, msgs, memo, feeGranter) {
  ...
  // 4th arg is the feeGranter: when a subscriber pays via a third-party fee
  // grant, the chain must charge the grantor's account, not the signer's.
  // Dropping it here silently billed the Keplr/Privy subscriber instead.
...
async function _safeBroadcastInner(msgs, memo, opts) {
  ...
  if (session.kind === 'keplr' || session.kind === 'privy') {
    const signDoc = await buildKeplrSignDoc(session, msgs, memo, opts && opts.feeGranter);
