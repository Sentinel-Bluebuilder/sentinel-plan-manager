...
function fmtP2P(udvpn) {
  ...
  // udvpn can exceed Number.MAX_SAFE_INTEGER (2^53) on whale balances, where
  // parseInt silently loses precision. Convert via BigInt for the integer part.
  try {
    const big = BigInt(String(udvpn).trim());
...
function clearCookie() {
  // Best-effort: a missing file (ENOENT) means already logged out, which is the
  // desired end state, so only surface unexpected errors.
  try {
    _unlinkSync(cookieFile());
  } catch (e) {
    if (e.code !== 'ENOENT') err(`WARN: could not clear cookie jar: ${e.message}`);
  }
}
...
async function request(method, path, body) {
  ...
  try {
    ...
    // Server returned a non-JSON body (HTML error page, empty 204, proxy error).
    // Keep the parse error so --json consumers can see why it failed.
    data = { error: `Non-JSON response (HTTP ${res.status}): ${e.message}` };
  }
...
async function cmdPlanCreate(f) {
  const gb = parseInt(f.gb, 10);
  const days = parseInt(f.days, 10);
  const priceUdvpn = parseInt(f['price-udvpn'], 10);
...
async function cmdLeaseStart(nodeAddr, f) {
  ...
  if (f.hours) body.hours = parseInt(f.hours, 10);
...
async function cmdProviderStatus(status) {
  ...
  const d = await POST('/api/provider/status', { status: parseInt(status, 10) });
  if (JSON_MODE) { printJson(d); return; }
  txLine(d);
}
...
async function cmdFeegrantGrant(grantee, f) {
  ...
  if (f['expiration-days']) body.expirationDays = parseInt(f['expiration-days'], 10);
...
async function cmdFeegrantGrantSubscribers(planId, f) {
  ...
  const body = { planId: parseInt(planId, 10) };
  if (f['spend-limit-dvpn']) body.spendLimitDvpn = parseFloat(f['spend-limit-dvpn']);
  if (f['expiration-days']) body.expirationDays = parseInt(f['expiration-days'], 10);
